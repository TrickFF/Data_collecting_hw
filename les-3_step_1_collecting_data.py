"""
Step 1.
В данном модуле производится скрапписнг сайтов SuperJob и HeadHunter
"""
...
vacancies = pd.DataFrame({'_id': [], 'site': [], 'name': [], 'min_wage': [], 'max_wage': [], 'currency': [], 'link': []})
...
for p in range(m):
    ...
    for i in range(len(elements['items'])):
        ...
        id_hh = 'hh' + elements['items'][i]['id']

        # Вносим данные в DataFame
        df_row = {'_id': id_hh, 'site': url_2, 'name': elements['items'][i]['name'], 'min_wage': min,
...
for i in range(n):
    ...
    for profession in profession_list_1:
        ...
        #Выводим ID вакансии
        id_1 =  str(profession_info_1['href'])[::-1].replace('lmth.', '')
        id_sj = ''
        for el in id_1:
            if el.isdigit():
                id_sj += el
        ...
        df_row = {'_id': id_sj, 'site': url_1, 'name': profession_name, 'min_wage': min,
